...
async function sendReadyForPickupEmail (order: OrderRow): Promise<void> {
  ...
  const text = `Hi ${order.customer_name || 'there'}, your order ${order.display_order_number} is ready for pickup.`
  ...
  try {
    ...
    // Email dispatch is optional and should not block status updates.
  }
}
...
export default defineEventHandler(async (event) => {
  ...
  const allowedStatuses = new Set(['preparing', 'ready', 'delivered', 'completed'])

  if (!status || !allowedStatuses.has(status)) {
    setResponseStatus(event, 400)
    return {
      ok: false,
      error: 'Status must be one of: preparing, ready, delivered, completed.'
    }
  }
  ...
  const storedStatus = status === 'completed' ? 'delivered' : status
  const now = new Date().toISOString()
  const deliveredAt = storedStatus === 'delivered' ? now : null
  ...
  const updateResult = await db
    ...
    .bind(storedStatus, preparingEmployeeId, deliveredAt, now, id)
    .run()
