  ...
  <section class="space-y-4">
    ...
    <div class="grid gap-4 md:grid-cols-2">
        ...
        <table class="mt-3 w-full text-sm">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.key" class="border-t border-gray-100">
              ...
              <td><input v-model="edits[item.key]" type="text" class="w-full rounded-md border border-gray-300 focus:border-brand focus:ring-brand" :disabled="saving || !canManage"></td>
      ...
      <form class="rounded-lg border border-gray-200 bg-white p-4 space-y-3" @submit.prevent="saveNew">
        ...
        <div><label class="text-sm font-medium text-gray-700">Value</label><input v-model="keyValue" type="text" class="mt-1 w-full rounded-md border border-gray-300 focus:border-brand focus:ring-brand" :disabled="saving || !canManage"></div>
